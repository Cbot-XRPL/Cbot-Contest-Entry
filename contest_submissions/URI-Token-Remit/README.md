...
|PARAM|NUMBER|  
|-----------|-------|  
|**URIL:**|The length in bytes of your base URI after it hex / 2. Use the XRPL Hex Visualizer Tool to convert your URIL to a uint64 before use in hook params. This needs to be set before adding the base URI. This can be updated as needed. Beware, if the URIL is not the correct URI length, your state saves and subsequent mints will fail. You can catch this by looking at your state with XRPL WIN Hook Tracking Tools and ensuring a consistent entry pattern.|  
